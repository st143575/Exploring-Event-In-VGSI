...
def parse(input_path:str, output_path:str):
    ...
    # Parse the rest 72221 steps(700000-772221).
	print("Parse the rest 72221 step headlines(700000-772221).\n")
    sid2parse_step_8 = dict()
    for sid in keys[700000:]:
        if sid not in sid2parse_step_8:
            print(sid, sid2step[sid])
            sid2parse_step_8[sid] = parser.predict(sid2step[sid], lang='en', prob=True, verbose=False)
            print(sid, "parsed.\n")
            
    # Write the deptrees for the rest 72221 step headlines to sid2stepdeptree_8.p file.
    print("Parsing finished. Start writting sid2stepdeptree_8.p file.\n")
    with open(output_path + '/sid2stepdeptree_8.p', 'wb') as file:
        pickle.dump(sid2parse_step_8, file)
    print("sid2stepdeptree_8.p writting finished.\n")
