...
def parse(input_path:str, output_path:str):
    ...
    # Parse the fourth 100,000 steps(300000-399999).
    print("Parse the fourth 100,000 step headlines(300000-399999).\n")
    sid2parse_step_4 = dict()
    for sid in keys[300000:400000]:
        if sid not in sid2parse_step_4:
            print(sid, sid2step[sid])
            sid2parse_step_4[sid] = parser.predict(sid2step[sid], lang='en', prob=True, verbose=False)
            print(sid, "parsed.\n")
            
    # Write the deptrees for the fourth 100,000 step headlines to sid2stepdeptree_4.p file.
    print("Parsing finished. Start writting sid2stepdeptree_4.p file.\n")
    with open(output_path + '/sid2stepdeptree_4.p', 'wb') as file:
        pickle.dump(sid2parse_step_4, file)
    print("sid2stepdeptree_4.p writting finished.\n")
