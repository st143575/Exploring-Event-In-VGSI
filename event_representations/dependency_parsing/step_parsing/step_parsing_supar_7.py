...
def parse(input_path:str, output_path:str):
    ...
    # Parse the seventh 100,000 steps(600000-699999).
	print("Parse the seventh 100,000 step headlines(600000-699999).\n")
    sid2parse_step_7 = dict()
    for sid in keys[600000:700000]:
        if sid not in sid2parse_step_7:
            print(sid, sid2step[sid])
            sid2parse_step_7[sid] = parser.predict(sid2step[sid], lang='en', prob=True, verbose=False)
            print(sid, "parsed.\n")
            
    # Write the deptrees for the seventh 100,000 step headlines to sid2stepdeptree_7.p file.
    print("Parsing finished. Start writting sid2stepdeptree_7.p file.\n")
    with open(output_path + '/sid2stepdeptree_7.p', 'wb') as file:
        pickle.dump(sid2parse_step_7, file)
    print("sid2stepdeptree_7.p writting finished.\n")
