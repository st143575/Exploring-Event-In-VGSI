...
def parse(input_path:str, output_path:str):
    ...
    # Parse the fifth 100,000 steps(400000-499999).
    print("Parse the fifth 100,000 step headlines(400000-499999).\n")
    sid2parse_step_5 = dict()
    for sid in keys[400000:500000]:
        if sid not in sid2parse_step_5:
            print(sid, sid2step[sid])
            sid2parse_step_5[sid] = parser.predict(sid2step[sid], lang='en', prob=True, verbose=False)
            print(sid, "parsed.\n")
            
    # Write the deptrees for the fifth 100,000 step headlines to sid2stepdeptree_5.p file.
    print("Parsing finished. Start writting sid2stepdeptree_5.p file.\n")
    with open(output_path + '/sid2stepdeptree_5.p', 'wb') as file:
        pickle.dump(sid2parse_step_5, file)
    print("sid2stepdeptree_5.p writting finished.\n")
